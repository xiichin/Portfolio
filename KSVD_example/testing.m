% tests K-SVD with artificial experiment
% Brian Ng
% 10/08/2020
...
Y = X;
%% Generate K-sparse D_test training data and Y_test,unseen data 
D_test = randn(N,K);
D_test = D_test./repmat(sqrt(sum(abs(D_test).^2,1)),N,1);
...
for it = 1:maxIts
    ...
    parfor ii = 1:L
        % Apply fista as alternative matching pursuit
         ...
         b = Y_test(:,ii);
         problem = lasso_problem(A,b,lambda);
         [W_new(:,ii),infos] = fista(problem, options, maxIts);
        
    end
    % apply SVD to update the trained dictionary 
    R = Y_test - D_trained*W_new;                                %  default R = Y - D*W;
    for k=1:K
        I = find(W_new(k,:));
        if ~isempty(I)
            % restricted error matrix
            Ri = R(:,I) + D_trained(:,k)*W_new(k,I);        % default Ri = R(:,I) + D(:,k)*W(k,I);
            [U,Sigma,V] = svds(Ri,1);               
            D_trained(:,k) = U;                             % default D(:,k) = U;
            W_new(k,I) = Sigma*V';
            R(:,I) = Ri - D_trained(:,k)*W_new(k,I);        % default R(:,I) = Ri - D(:,k)*W(k,I);
        end
    end
...
E_trained = X_test-D_trained*W_new;
usage_trained = zeros(K,1);

parfor k_trained = 1:K
    usage_trained(k_trained) = numel(find(W_new(k_trained,:)));
end
...
parfor m = 1:L
    ss_trained(m) = numel(find(abs(W_new(:,m)) > 0));
end
...
%% Plots
figure(1);
plot(1:maxIts,rmse,'b'); grid on;
xlabel('Iteration'); ylabel('RMSE');
title('True Dictionary (RMSE vs iteration)');

figure(2);
plot(1:maxIts,rmse_test,'b'); grid on;
xlabel('Iteration'); ylabel('RMSE');
title('Trained Dictionary (RMSE vs iteration)');

figure(3);
imagesc(D_angle); colorbar;
title('Atom angle separation (degrees) between trained and true dictionaries');
...
figure(5);
imagesc(abs(W)>0);
title('True Dictionary(Sparse coefficient)');

figure(6);
imagesc(abs(W_new)>0);
title('Trained Dictionary(Sparse coefficient)');
